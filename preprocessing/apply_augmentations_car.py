...
def apply_random_from_group(image, group_id, image_path=None):
    if group_id == 1:
        func = random.choice([adjust_brightness, apply_color_temperature, apply_desaturation])
        return func(image)
    elif group_id == 2:
        return random.choice([adjust_contrast_blend, adjust_contrast, rotate])(image)
    elif group_id == 3:
    ...
    for filename in tqdm(image_files, desc="Augmenting Images"):
        if filename.lower().endswith(('.jpg', '.jpeg', '.png')):
            ...
            for i in range(multiplier):
                ...
                chosen_groups = random.sample(range(1, 6), num_augments)
...
target_numbers = [12000,12000,12000,12000,12000,12000,12000,12000,12000,12000,3000,12000,3140]
